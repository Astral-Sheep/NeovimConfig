...
function M.info()
	...
	lines[#lines + 1] = "```"
	Config.utils.info(lines, { title = "Config Roots" })
...
function M.setup()
	vim.api.nvim_create_user_command("Root", function()
		Config.root.info()
	end, { desc = "Config roots for the current buffer" })
	...
	-- Maybe this is too frequen on `BufEnter` and something else should be done instead??
...
function M.get(opts)
	...
	return Config.utils.is_win() and ret:gsub("/", "\\") or ret
end
