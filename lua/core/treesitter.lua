...
---@return boolean ok, core.treesitter.Health health
function M.check()
	...
	---@class core.treesitter.Health: table<string, boolean>
	local ret = {
...
function M.build(cb)
	M.ensure_treesitter_cli(function(_, err)
		...
		if ok then
			...
			Config.utils.error(lines, { title = "LazyVim Treesitter" })
		end
	end)
end
...
function M.ensure_treesitter_cli(cb)
	...
	mr.refresh(function()
		...
		if not p:is_installed() then
			Config.utils.info("Installing `tree-sitter-cli` with `mason.nvim`...")
			p:install(
				nil,
				vim.schedule_wrap(function(success)
					if success then
						Config.utils.info("Installed `tree-sitter-cli` with `mason.nvim`.")
						cb(true)
					else
...
function M.apply_overrides()
	...
	local overrides = (colorschemes[vim.g.colors_name] ~= nil and colorschemes[vim.g.colors_name].theme_overrides) or {}
