...
---@alias core.cmp.Action fun():boolean?
---@type table<string, core.cmp.Action>
M.actions = {
	-- Native Snippets
...
-- This funciton replaces nested placeholders in a snippet with LSP placeholders.
...
function M.expand(snippet)
	...
	-- Otherwise, when on the first placeholder and selection a new completion,
...
function M.setup(opts)
	...
	require('cmp.utils.snippet').parse = function(input)
		...
		return Config.cmp.snippet_preview(input)
	end
	...
	cmp.event:on('confirm_done', function(event)
		if vim.tbl_contains(opts.auto_brackets or {}, vim.bo.filetype) then
			Config.cmp.auto_brackets(event.entry)
		end
	end)
	cmp.event:on('menu_opened', function(event)
		Config.cmp.add_missing_snippet_docs(event.window)
	end)
end
